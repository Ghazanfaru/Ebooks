            ...
            children: [
              ...
              SizedBox(
                ...
                child: StreamBuilder<QuerySnapshot<Map<String,dynamic>>>(
                    stream: store.collection('audio').where('category',isEqualTo: category).snapshots(),
              ...
              InkWell(onTap: ()=>ShowCategory2(),
                  child: TextButtons(text: category2, btntxt: 'More',)),
              SizedBox(
                height: 260,
                child: StreamBuilder<QuerySnapshot<Map<String,dynamic>>>(
                    stream: store.collection('audio').where('category',isEqualTo: category2).snapshots(),
                    builder:(context,AsyncSnapshot snapshot){
                      if(snapshot.connectionState==ConnectionState.waiting)
                      {return const Center(child: CircularProgressIndicator(color: Colors.green,));}
                      else if(snapshot.connectionState==ConnectionState.active){
